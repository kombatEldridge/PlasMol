...
def absorption_post_process_single(field_p_filepath, component, params, ref_e_filepath=None):
    """
    ...
    if params.has_plasmon:
        if params.absorption_use_existing_e_field_ref:
            ...
            logger.info(
                ...
                f"reference E_inc '{params.absorption_field_e_ref_filepath}'."
            )
        else:
            ...
            logger.info(
                ...
                f"E_inc written to '{params.absorption_field_e_ref_filepath}'."
            )

        if not np.isclose(params.absorption_tau, 0):
            field_e = apply_tau_damping_arrays(time_e, field_e, params.absorption_tau)
            logger.info(f"Applied tau={params.absorption_tau} damping to reference E_inc (in memory).")
    ...
    abs_imag, freqs = fourier(
        time_points,
        dipole_moment,
        params.absorption_gamma,
        params.absorption_min_ev,
        params.absorption_max_ev,
