...
BROKEN_RE = re.compile(r"self\.mf\.omega\s*=\s*0\b")
FIXED_RE = re.compile(
    r"self\.mf\.omega\s*=\s*self\.molecule_lrc_parameter"
)
...
def pair_covers_edit(broken: str, fixed: str, edit: dict) -> bool:
    return is_broken_line(broken) and is_fixed_line(fixed) and "mf.omega" in edit["old"]
