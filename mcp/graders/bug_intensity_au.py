...
BROKEN_RE = re.compile(
    r"active_component\[mask\]\s*=\s*1e5\s*\*\s*self\.intensity_au"
)
FIXED_RE = re.compile(r"active_component\[mask\]\s*=\s*self\.intensity_au")


def is_broken_line(line: str) -> bool:
    return bool(BROKEN_RE.search(line)) and "1e5" in line


def is_fixed_line(line: str) -> bool:
    return bool(FIXED_RE.search(line)) and "1e5" not in line
...
def outputs_match(restored: str, canonical: str) -> bool:
    return bool(FIXED_RE.search(restored)) and "1e5" not in restored


def pair_covers_edit(broken: str, fixed: str, edit: dict) -> bool:
    return is_broken_line(broken) and is_fixed_line(fixed) and "intensity_au" in edit["old"]
